...
public class VnPayController {
    ...
    public VnPayController(VnPayServiceImpl vnPayService, ResponseMapper responseMapper, WalletServiceImpl walletServiceImpl) {
...
//    }
    ...
    public ResponseEntity<?> createPayment(HttpServletRequest request, @RequestParam long amount) {
        try {
            ...
            return ResponseEntity.ok(responseMapper.toDto(true, "Tạo liên kết thanh toán thành công.", result, null));
        } catch (Exception e) {
            JsonObject error = new JsonObject();
            error.addProperty("code", "99");
            error.addProperty("message", "error: " + e.getMessage());
            return ResponseEntity.badRequest().body(responseMapper.toDto(false, "Không thể tạo link thanh toán.", error, e));
        }
    }
    ...
    public ResponseEntity<?> handleVnPayReturn(HttpServletRequest request) {
        ...
        if(result.get("response_code").equals("00")) {
            ...
            return ResponseEntity.ok(responseMapper.toDto(true, "Nạp tiền thành công.",
                    wallet, null));
        }
        return ResponseEntity.ok(responseMapper.toDto(false, "Nạp tiền không thành công.",
    ...
    public ResponseEntity<?> ipn(HttpServletRequest request) {
        ...
        Wallet wallet = walletServiceImpl.processDepositMoneyIntoWallet(inputData);
        return ResponseEntity.ok(responseMapper.toDto(true, "Nạp tiền thành công.", wallet, null));
    }
